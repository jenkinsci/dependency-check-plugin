...
function generateSeverityDistributionBar(targetId, label, critical, high, medium, low, info, unassigned) {
    ...
    if (critical === 0 && high === 0 && medium === 0 && low === 0 && info === 0 && unassigned === 0) {
        block += '<div class="severity-distribution-bar severity-info-bg" tooltip="No Vulnerabilities Found" style="width:100%">No Vulnerabilities Found</div>';
    } else {
        block += '<div class="severity-distribution-bar severity-critical-bg" tooltip="Critical: ' + critical + ' (' + Math.round(percentCritical*10)/10 + '%)" style="width:' + percentCritical+ '%">' + critical + '</div>';
        block += '<div class="severity-distribution-bar severity-high-bg" tooltip="High: ' + high + ' (' + Math.round(percentHigh*10)/10 + '%)" style="width:' + percentHigh + '%">' + high + '</div>';
        block += '<div class="severity-distribution-bar severity-medium-bg" tooltip="Medium: ' + medium + ' (' + Math.round(percentMedium*10)/10 + '%)" style="width:' + percentMedium + '%">' + medium + '</div>';
        block += '<div class="severity-distribution-bar severity-low-bg" tooltip="Low: ' + low + ' (' + Math.round(percentLow*10)/10 + '%)" style="width:' + percentLow + '%">' + low + '</div>';
        block += '<div class="severity-distribution-bar severity-info-bg" tooltip="Info: ' + info + ' (' + Math.round(percentInfo*10)/10 + '%)" style="width:' + percentInfo + '%">' + info + '</div>';
        block += '<div class="severity-distribution-bar severity-unassigned-bg" tooltip="Unassigned: ' + unassigned + ' (' + Math.round(percentUnassigned*10)/10 + '%)" style="width:' + percentUnassigned + '%">' + unassigned + '</div>';
    }
    block += '</div>';
    document.getElementById(targetId).innerHTML = block;
    Behaviour.applySubtree(document.getElementById(targetId));
}
